class AdminUser
    ...
    def self.find_by_apikey(params)
     user = Api::Accounts.new.where(params)
     if user
         return AdminUser.new_from_params(user.expanded)
     end
    end
    ...
    def hash_password(password)
        raise "password is too long" if password.size > AdminUser.max_password_length
        Base64.strict_encode64(password)
    end

    def dehash_password
        Base64.strict_decode64(password_hash)
    end


    def admin?
        if authority
          authority.include?("admin")
