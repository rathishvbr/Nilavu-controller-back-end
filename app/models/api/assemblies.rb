...
module Api
    class Assemblies < ApiDispatcher
        ...
        ## this should be build int common_deployable..We are underutilizing that.
        def build_data(api_params)
            {   name: '',
                assemblies: [Megam::Mixins::Assemblies.new(api_params).to_hash],
                inputs: [units_to_spin(api_params)],
        ...
        #do you want to tell an error ! Nah, it gets trapped at VerticeLauncher itself.
        def units_to_spin(params)
          { key: "number_of_units", value: params[:number_of_units] }
        end
        ...
        def dig_assembly(tmp_assemblies_collection, api_params)
            ...
            @baked.select!{|a| !valid_states.include?(a[0][0].state)}
