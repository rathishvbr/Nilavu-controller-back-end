...
class CephUser
  ...
  def save
   # if saved = GWUser.save(@email, CephUser.suggest_name(email))
    if saved = bildr_user_is_ready.new.save(@email, CephUser.suggest_name(email))
  ...
  def email_available?
   # GWUser.exists?(@email)
    bildr_user_is_ready.exists?(@email)
  ...
  def bildr_user_is_ready
      bildr = BackupRestore::UserProvider.new
  ...
  def find
    if email_available?
     # return CephUser.new_from_params(GWUser.find_by_email(@email))
      return CephUser.new_from_params(bildr_user_is_ready.new.find_by_email(@email))
    end
  end
