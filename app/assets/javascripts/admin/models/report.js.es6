    ...
    valueAt(numDaysAgo) {
        if (this.data) {
            ...
            const item = this.data.find(d =>
             moment(d.filterBy('key','x')[0].value).format("YYYY-MM-DD") === wantedDate
            );
            if (item) {
                return item.filterBy('key','y')[0].value;
            };
        }
        return 0;
    },
    ...
    valueFor(startDaysAgo, endDaysAgo) {
        if (this.data) {
            ...
            _.each(this.data, datum => {
                d = moment(datum.filterBy('key','x')[0].value);
                if (d >= earliestDate && d <= latestDate) {
                    sum += parseInt(datum.filterBy('key','y')[0].value);
                    count++;
                }
            });
    ...
    find(type, startDate, endDate, categoryId, groupId) {
        return Nilavu.ajax("/admin/reports/" + type, {
            ...
            json.report.data.forEach(row => {
                if (row.filterBy('key','y')[0].value > maxY)
                    maxY = row.filterBy('key','y')[0].value;
                }
            );
            if (maxY > 0) {
                json.report.data.forEach(row => row.percentage = Math.round((row.filterBy('key','y')[0].value / maxY) * 100));
            }
