...
export default RestModel.extend({
    ...
    @computed('posts.[]')hasPosts() {
        return this.get('posts.length') > 0;
    },

    @computed('hasPosts', 'filteredPostsCount')hasLoadedData(hasPosts, filteredPostsCount) {
    ...
    @computed('hasLoadedData', 'firstPostId', 'posts.[]')firstPostPresent(hasLoadedData, firstPostId) {
        if (!hasLoadedData) {
            return false;
        }
    ...
    @computed('hasLoadedData', 'lastPostId', 'posts.@each.id')loadedAllPosts(hasLoadedData, lastPostId) {
        if (!hasLoadedData) {
            return false;
        }
    ...
    **/
    @computed('summary', 'show_deleted', 'userFilters.[]')streamFilters(summary, showDeleted) {
        const result = {};
        if (summary) {
    ...
    @computed('streamFilters.[]', 'topic.posts_count', 'posts.length')hasNoFilters() {
    ...
    **/
    @computed('posts.[]', 'stream.[]')previousWindow() {
    ...
    **/
    @computed('posts.lastObject', 'stream.[]')nextWindow(lastLoadedPost) {
        // If we can't find the last post loaded, bail
        if (!lastLoadedPost) {
            return [];
        }
