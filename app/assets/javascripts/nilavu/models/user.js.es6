...
const User = RestModel.extend({
    ...
    approvalDone: function() {
        const approved = this.get('approved') || "";
        return approved.trim().length > 0
            ? approved.trim() === "true"
            : false;
    }.property('approved'),

    knockedOff: function() {
        return this.get('activated') && this.get('approvalDone');
    },
