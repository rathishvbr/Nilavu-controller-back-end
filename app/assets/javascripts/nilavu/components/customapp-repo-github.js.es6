import {reposForUser} from 'nilavu/lib/github';
...
export default Ember.Component.extend({
    ...
    repoDetail: null,
    //allow customer to directly login to github when choose the customapp
    ...
    repos: function() {
        ...
        if (this.get('resultRepos') && this.get('resultRepos').length > 0) {
            this.get('resultRepos').forEach(function(repo) {
                result.push({name: repo.clone_url, value: repo.clone_url});
            });
        }
        return result;
    ...
    refreshRepo: function() {
        ...
        if (!this._repos) {
            this.notificationMessages.error(I18n.t('customapp.github_error'));
            return;
        }
