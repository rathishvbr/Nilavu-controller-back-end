export default Em.Component.extend({
    ...
    submitDisabled: function() {
        if (!this.get('category.versionoption'))
            return;
        return ((this.get('category.versionoption') || this.get('category.versionoption').length >= 1) && (this.get('category.versionoption') === this.get('resultType.name').toLowerCase()));
    ...
    actions: {
        ...
        edit() {
            ...
            if (this.get('resultType.options')) {
                ...
                if (!Em.isEmpty(usrl)) {
                    this.set('category.sourceurl', usrl.get('firstObject').value);
                }
            }
            this.set('versionOption', true);
            this.toggleProperty('gotoSummarize');
        }
