...
export default Ember.Component.extend({
    ...
    @observes('searchService.searchContext')contextChanged: function() {
    ...
    @computed('searchService.searchContext', 'searchService.term', 'searchService.searchContextEnabled')fullSearchUrlRelative(searchContext, term, searchContextEnabled) {
    ...
    @computed('fullSearchUrlRelative')fullSearchUrl(fullSearchUrlRelative) {
        if (fullSearchUrlRelative) {
            return Nilavu.getURL(fullSearchUrlRelative);
        }
    },

    @computed('searchService.searchContext')searchContextDescription(ctx) {
        return searchContextDescription(Em.get(ctx, 'type'), Em.get(ctx, 'user.username') || Em.get(ctx, 'category.name'));
    },

    @observes('searchService.searchContextEnabled')searchContextEnabledChanged() {
        if (_dontSearch) {
            return;
        }
    ...
    @observes('searchService.term', 'typeFilter')newSearchNeeded() {
    ...
    searchTerm(term, typeFilter) {
        ...
        const searchContext = this.get('searchService.searchContext')
            ? this.get('searchService.searchContext')
            : null;
        this._search = searchForTerm(term, {typeFilter, searchContext, fullSearchUrl: this.get('fullSearchUrl')});
        this._search.then((content) => {
            this.setProperties({
                noResults: content.grouped_search_result.length === 0,
                content
            });
        }). finally(() => {
            this.set('loading', false);
    ...
    @computed('typeFilter', 'loading')showCancelFilter(typeFilter, loading) {
        if (loading) {
            return false;
        }
    ...
    @observes('searchService.term')termChanged() {
