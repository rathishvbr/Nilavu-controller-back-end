...
const controllerOpts = {
    ...
    actions: {
        ...
        createTopic() {
            ...
            this.openComposer().then(function(result) {
    ...
    isFilterPage: function(filter, filterType) {
        ...
        return filter.match(new RegExp(filterType + '$', 'gi'))
            ? true
            : false;
    },
    ...
    footerEducation: function() {
        if (!this.get('allLoaded') || this.get('model.topics.length') > 0 || !Nilavu.User.current()) {
            return;
        }
