...
require 'root_password'


class SshKeysController < ApplicationController
    respond_to :html, :js, :json

    before_action :add_authkeys_for_api, only: [:index, :create, :edit, :import, :show, :pwdUpdate]
    ...
    def pwdUpdate
        ...
        params[:outputs] = params[:outputs].values if params[:outputs].present?
        base_root_password = Base64.strict_encode64(root_password)
        params[:inputs] = inputs.push({"key":"root_password","value":base_root_password})
